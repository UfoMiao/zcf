...
function uninstallToolName(): string {
  return i18n.t(claudeDefinition.displayNameKey)
}
 ...
 */
export async function runLegacyClaudeCodeUninstall(options: UninstallOptions = {}): Promise<void> {
  try {
...
async function showInteractiveUninstall(uninstaller: ZcfUninstaller): Promise<void> {
  ...
  const { mainChoice } = await inquirer.prompt<{ mainChoice: 'complete' | 'custom' }>({
    ...
    choices: addNumbersToChoices([
      {
        name: `${i18n.t('uninstall:completeUninstall')} - ${ansis.gray(i18n.t('uninstall:completeUninstallDesc', { tool: uninstallToolName() }))}`,
        value: 'complete',
...
async function executeCompleteUninstall(uninstaller: ZcfUninstaller): Promise<void> {
  ...
  console.log(ansis.yellow(i18n.t('uninstall:completeWarning', { tool: uninstallToolName() })))

  // Final confirmation
  const confirm = await promptBoolean({
    message: i18n.t('uninstall:confirmComplete', { tool: uninstallToolName() }),
    defaultValue: false,
  })
