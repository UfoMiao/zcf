import type { CodeBuddyProfile } from '../../../../src/types/codebuddy-config'
...
vi.mock('../../../../src/utils/config.model-keys', () => ({
  clearCodeBuddyEnv: vi.fn((env: Record<string, string>) => {
    delete env.CODEBUDDY_API_KEY
    delete env.CODEBUDDY_AUTH_TOKEN
    delete env.CODEBUDDY_BASE_URL
    delete env.CODEBUDDY_MODEL
    delete env.CODEBUDDY_SMALL_FAST_MODEL
    delete env.CODEBUDDY_BIG_SLOW_MODEL
    delete env.CODEBUDDY_CODE_SUBAGENT_MODEL
  }),
}))
  ...
  describe('applyProfileSettings', () => {
    ...
    it('should write all model env variables to settings.json env', async () => {
      ...
      const profile: CodeBuddyProfile = {
        ...
        bigSlowModel: 'claude-opus-4',
        codeSubagentModel: 'claude-sonnet-4',
      }
      ...
      const call = mockJsonConfig.writeJsonConfig.mock.calls[0] as [string, any]
      expect(call[1].env.CODEBUDDY_API_KEY).toBe('sk-test')
      expect(call[1].env.CODEBUDDY_BASE_URL).toBe('https://api.example.com')
      expect(call[1].env.CODEBUDDY_MODEL).toBe('claude-sonnet-4')
      expect(call[1].env.CODEBUDDY_SMALL_FAST_MODEL).toBe('claude-haiku-4')
      expect(call[1].env.CODEBUDDY_BIG_SLOW_MODEL).toBe('claude-opus-4')
      expect(call[1].env.CODEBUDDY_CODE_SUBAGENT_MODEL).toBe('claude-sonnet-4')
    })
    ...
    it('should not set API key env for unsupported authType', async () => {
      ...
      expect(call[1].env.CODEBUDDY_AUTH_TOKEN).toBeUndefined()
    })
    ...
    it('should only set provided optional fields', async () => {
      ...
      const call = mockJsonConfig.writeJsonConfig.mock.calls[0] as [string, any]
      expect(call[1].env.CODEBUDDY_API_KEY).toBe('sk-test')
      expect(call[1].env.CODEBUDDY_BASE_URL).toBeUndefined()
      expect(call[1].env.CODEBUDDY_MODEL).toBeUndefined()
    })
  ...
  describe('sanitizeProfile', () => {
    it('should keep only known fields', () => {
      const profile: CodeBuddyProfile = {
        ...
        bigSlowModel: 'claude-opus-4-5',
        codeSubagentModel: 'claude-sonnet-4-5',
      }
      ...
      expect(sanitized.smallFastModel).toBe('claude-haiku-4-5')
      expect(sanitized.bigSlowModel).toBe('claude-opus-4-5')
      expect(sanitized.codeSubagentModel).toBe('claude-sonnet-4-5')
    })
  })
})
