...
describe('resolveCodeType', () => {
  ...
  it('should resolve oc abbreviation to opencode', async () => {
    const result = await resolveCodeType('oc')
  ...
  it('should accept full code type names', async () => {
    ...
    const result3 = await resolveCodeType('opencode')
    expect(result3).toBe('opencode')
  })
  ...
  it('should be case insensitive', async () => {
    ...
    const result3 = await resolveCodeType('OC')
    expect(result3).toBe('opencode')
  })

  it('should throw error for invalid code type', async () => {
    await expect(resolveCodeType('invalid')).rejects.toThrow(
      'Invalid code type: "invalid". Valid options are: cc, cx, oc, claude-code, codex, opencode. Using default: codex.',
    )
  })
  ...
  it('should use DEFAULT_CODE_TOOL_TYPE when config read fails in error path', async () => {
    ...
    await expect(resolveCodeType('invalid')).rejects.toThrow(
      'Invalid code type: "invalid". Valid options are: cc, cx, oc, claude-code, codex, opencode. Using default: claude-code.',
    )
  })
  ...
  it('should handle invalid config value in error path', async () => {
    ...
    await expect(resolveCodeType('wrong')).rejects.toThrow(
      'Invalid code type: "wrong". Valid options are: cc, cx, oc, claude-code, codex, opencode. Using default: claude-code.',
    )
  })
})
