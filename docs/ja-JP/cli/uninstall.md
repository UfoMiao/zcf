...
`zcf uninstall` は安全なアンインストールプロセスを提供し、ZCF 設定のみのクリーンアップ、選択的アンインストール、完全アンインストール、競合解決をサポートします。環境をリセット、デバイスを移行、または設定をクリーンアップする必要があるシナリオに適しています。
...
3. 🧹 **ZCF 設定のみのクリーンアップ**：Claude Code または Codex CLI、ユーザー設定、バックアップを保持したまま、ZCF 管理の設定を削除
4. 💾 **バックアップ保持**：復元のためにバックアップを保持することをサポート
5. 🔍 **競合検出**：ファイル競合を検出して解決
6. 🗂️ **ゴミ箱サポート**：システムゴミ箱を使用して安全に削除（macOS、Windows、Linux をサポート）
...
1. アンインストールモードを選択（完全、ZCF 設定のみ、またはカスタムアンインストール）
...
ZCF が管理する設定だけを削除し、コードツール CLI、無関係なユーザー設定、バックアップを保持します。Claude Code と Codex の両方に対応しています：

所有権マーカーと設定値のフィンガープリントで ZCF リソースと同名のユーザーリソースを区別し、未マークまたはユーザーが変更したリソースは削除しません。
...
|------|------|--------|--------|
| `--mode, -m` | アンインストールモード | `complete`, `zcf`, `custom`, `interactive` | `interactive` |
